...
function renderSeason(anno) {
  ...
  classifica.forEach((team, i) => {
    ...
    const secondi = season.classifica.filter((t) => t.squadra === team.squadra && t.secondoAllenatore);
    const misterLabel = secondi.length > 0 ? `${team.mister} / ${secondi[0].mister}` : team.mister || "N/A";
...
const FASCE_EXTRA = {
  "2024-25": {
    startNote: "Prima stagione con il sistema a fasce. Le fasce di partenza erano ufficiosamente basate sulla stagione 2023-24.",
    mosse: {},
  },
  "2025-26": {
    ...
    notePartenza: {
      "Alex Beltrame": "Subentrato a Kevin Sandri (ritirato). Avrebbe dovuto partire da F3 per il 7° posto 24/25, promosso per subentrare al posto di Sandri.",
    ...
    noteDestinazione: {
      "Federico Burello":  "Campione della stagione 2025-26!",
...
function buildMisterStats() {
  ...
  if (!stats["Valentina Pozzi"].stagioni.find((x) => x.anno === "2025-26" && x.squadra === "Juventus")) {
    stats["Valentina Pozzi"].stagioni.push({ anno: "2025-26", annoInizio: 2025, squadra: "Juventus", logo: "images/juventus.png", pos: 5, parziale: false, isVice: true, secondoAllenatore: false, uscita: false, nota: "Viceallenatrice della Juventus" });
  }

  if (!stats["Giacomo Bot"]) stats["Giacomo Bot"] = { stagioni: [] };
  if (!stats["Giacomo Bot"].stagioni.find((x) => x.anno === "2023-24" && x.squadra === "Napoli")) {
    stats["Giacomo Bot"].stagioni.push({ anno: "2023-24", annoInizio: 2023, squadra: "Napoli", logo: "images/napoli.png", pos: 8, parziale: false, isVice: true, secondoAllenatore: false, uscita: false, nota: "Viceallenatore di Mattia Minin" });
  }
  if (!stats["Giacomo Bot"].stagioni.find((x) => x.anno === "2024-25" && x.squadra === "Bologna")) {
    stats["Giacomo Bot"].stagioni.push({ anno: "2024-25", annoInizio: 2024, squadra: "Bologna", logo: "images/bologna.png", pos: 4, parziale: false, isVice: true, secondoAllenatore: false, uscita: false, nota: "Viceallenatore di Federico Burello" });
  }

  if (!stats["Kevin Sandri"]) stats["Kevin Sandri"] = { stagioni: [] };
  if (!stats["Kevin Sandri"].stagioni.find((x) => x.anno === "2025-26" && x.squadra === "Bologna")) {
    stats["Kevin Sandri"].stagioni.push({ anno: "2025-26", annoInizio: 2025, squadra: "Bologna", logo: "images/bologna.png", pos: 8, parziale: false, isVice: true, secondoAllenatore: false, astaSolo: true, uscita: false, nota: "Ha svolto l'asta estiva per Nicola Marano (Bologna), lasciando poi la squadra completamente in mano a Nico come 1° allenatore — non ha disputato attivamente il campionato" });
  }

  if (!stats["Aidan Conti"]) stats["Aidan Conti"] = { stagioni: [] };
  if (!stats["Aidan Conti"].stagioni.find((x) => x.anno === "2025-26" && x.squadra === "Lazio")) {
    stats["Aidan Conti"].stagioni.push({ anno: "2025-26", annoInizio: 2025, squadra: "Lazio", logo: "images/lazio.png", pos: 7, parziale: false, isVice: true, secondoAllenatore: false, uscita: false, nota: "Ha svolto l'asta estiva alla guida della Lazio, diventando viceallenatore di Cristian Tartaro per la stagione 2025/26" });
  }
  if (!stats["Aidan Conti"].stagioni.find((x) => x.anno === "2026-27")) {
    stats["Aidan Conti"].stagioni.push({ anno: "2026-27", annoInizio: 2026, squadra: "TBD", logo: "images/default.png", pos: null, parziale: false, isVice: false, secondoAllenatore: false, futuro: true, nota: "Confermata presenza come fantallenatore di Serie A — squadra ancora da definire" });
  }
...
function renderHallOfFame(targetContainer) {
  ...
  const FIXED_ORDER = ["Denis Mascherin","Mattia Beltrame","Kevin Di Bernardo","Federico Burello","Cristian Tartaro","Alex Beltrame","Lorenzo Moro","Nicola Marano","Aidan Conti","Valentina Pozzi","Kevin Sandri","Andrea Campagnolo","Giovanni Bean","Giacomo Bot","Mattia Minin","Riccardo Rella","Michele Picilli"];
  const allNames = Object.keys(stats);
  const orderedNames = [...FIXED_ORDER.filter((n) => allNames.includes(n)), ...allNames.filter((n) => !FIXED_ORDER.includes(n))];
    ...
    const teamsHtml = stagioni.map((s) => `
        <div class="mister-team-entry">
          ...
          <div class="mister-team-info">
            ...
            <span class="mister-team-pos">${s.futuro ? '<span class="badge-futuro">🔜 Prossima</span>' : s.astaSolo ? '<span class="badge-asta">🎯 Asta</span>' : s.isVice ? '<span class="badge-vice">VICE</span>' : s.parziale ? '<span class="badge-parziale">Parziale</span>' : getPosLabel(s.pos)}</span>
          </div>
        </div>`).join("");
...
function createMisterModal() {
  ...
  modal.addEventListener("click", function (e) { if (e.target === modal) closeMisterModal(); });
...
function openMisterModal(encodedName) {
  ...
  const coppePartecipazioni = (fantaData.coppa || []).map((c) => {
    const entry = (c.classifica || []).find((r) => r.mister && r.mister.split("(")[0].trim() === name);
    if (!entry) return null;
    return { stagione: c.stagione, squadra: entry.squadra, logo: entry.logo, fase: entry.fase, pos: entry.pos, inCorso: c.risultato === "In corso", coppaNote: entry.coppaNote || null };
  }).filter(Boolean);

  const torneiPartecipazioni = (fantaData.tornei || []).map((t) => {
    const entry = (t.classifica || []).find((r) => r.mister && r.mister.split("(")[0].trim() === name);
    const futuro = (t.partecipanti || []).find((p) => p.mister === name);
    if (!entry && !futuro) return null;
    if (futuro && !entry) return { tipo: t.tipo, edizione: t.edizione, nazione: "TBD", logo: "images/default.png", fase: "In corso", punti: null, pos: null, futuro: true };
  ...
  const stagionDetailHtml = stagioni.map((s) => `
    <div class="modal-season-row">
      ...
      <span class="modal-season-pos">${s.futuro ? '<span class="badge-futuro">🔜 Prossima</span>' : s.astaSolo ? '<span class="badge-asta">🎯 Asta</span>' : s.isVice ? '<span class="badge-vice">VICE</span>' : getPosLabel(s.pos)}</span>
    </div>`).join("");
  ...
  document.getElementById("misterModalContent").innerHTML = `
    ...
    <div class="modal-stats-row">
      <div class="modal-stat-box"><div class="modal-stat-value">${partecipazioni}</div><div class="modal-stat-label">Partecipazioni</div></div>
      <div class="modal-stat-box highlight-gold"><div class="modal-stat-value">${titoli.length}</div><div class="modal-stat-label">Titoli Vinti</div></div>
      <div class="modal-stat-box"><div class="modal-stat-value">${podi}</div><div class="modal-stat-label">Podi Totali</div></div>
    </div>
    ...
    ${coppePartecipazioni.length > 0 ? `
      ...
      <div class="modal-trophies-list">${coppePartecipazioni.map((c) => {
        ...
        return `<div class="modal-trophy-entry coppa-entry partecipazione-entry">
    ...
    ${torneiPartecipazioni.length > 0 ? `
      ...
      <div class="modal-trophies-list">${torneiPartecipazioni.map((t) => {
        const icon = t.tipo === "FantaMundial" || t.tipo === "FantaMundiale" ? "🌍" : "⭐";
        const tClass = t.tipo === "FantaMundial" || t.tipo === "FantaMundiale" ? "torneo-entry mondiale-entry" : "torneo-entry europeo-entry";
        const faseColor = t.fase === "Campione" ? "fase-campione" : t.fase === "Finale" ? "fase-finale" : t.fase === "In corso" ? "fase-incorso" : "fase-eliminato";
        const ptsLabel = t.punti !== null && t.punti !== undefined ? ` — ${t.punti} pt` : "";
        const nazioneLabel = t.futuro ? "Partecipazione confermata" : `${t.nazione}${ptsLabel}`;
        return `<div class="modal-trophy-entry ${tClass} partecipazione-entry">
            <img src="${t.logo}" class="modal-trophy-logo" onerror="this.src='images/default.png'">
            <div class="partecipazione-info">
...
function changeSeason(val) { renderSeason(val); }
...
function togglePalmares() {
  const hidden = document.getElementById("palmaresSection").classList.contains("hidden");
  resetAllSections();
  if (hidden) { document.getElementById("palmaresSection").classList.remove("hidden"); document.querySelector(".btn-palmares").innerText = "Torna alla Classifica"; }
  else { document.getElementById("rankingSection").classList.remove("hidden"); }
}

function toggleAttivita() {
  const hidden = document.getElementById("attivitaSection").classList.contains("hidden");
  resetAllSections();
  if (hidden) { document.getElementById("attivitaSection").classList.remove("hidden"); document.querySelector(".btn-attivita").innerText = "Torna alla Classifica"; renderAttivita(); }
  else { document.getElementById("rankingSection").classList.remove("hidden"); }
}
...
function renderAttivita() {
  ...
  ordered.forEach((name) => {
    ...
    if (isAdmin && !staffDone) { staffDone = true; html += '<div class="attivita-group-label attivita-group-admin">⚙️ Staff</div>'; }
    if (!isAdmin && isActive && !activeDone) { activeDone = true; html += '<div class="attivita-group-label attivita-group-active">🟢 Attivi</div>'; }
    if (!isAdmin && !isActive && !retiredDone) { retiredDone = true; html += '<div class="attivita-group-label attivita-group-retired">🔴 Ritirati</div>'; }

    const endYearStr = isActive ? "in corso" : (() => { const parts = maxAnno.split("-"); const y2 = parts[1]; return y2.length === 2 ? "20" + y2 : y2; })();
    ...
    const roleBadge = name === "Denis Mascherin" ? '<span class="attivita-role-badge admin">👑 SuperAdmin</span>' : name === "Kevin Di Bernardo" ? '<span class="attivita-role-badge vice">🛡️ ViceAdmin</span>' : isOnlyVice ? '<span class="attivita-role-badge only-vice">🔁 Solo Vice</span>' : "";
    const cardClass = isAdmin ? " attivita-card-admin" : isOnlyVice ? " attivita-card-vice" : isUscita && isActive ? " attivita-card-uscita" : "";
    const avatarClass = isAdmin ? " attivita-avatar-admin" : isOnlyVice ? " attivita-avatar-vice" : isUscita && isActive ? " attivita-avatar-uscita" : "";
    const statusClassFinal = isActive ? isOnlyVice ? "attivita-status active-vice" : isUscita ? "attivita-status uscita" : "attivita-status active" : isOnlyVice ? "attivita-status retired-vice" : "attivita-status retired";
    const statusText = isActive ? isOnlyVice ? "🔁 Vice Attivo" : isUscita ? "🟡 Ultima Stagione" : "🟢 Attivo" : isOnlyVice ? "🔁 Vice Ritirato" : "🔴 Ritirato";
    const stagionLabel = isOnlyVice ? `${stagioni.filter((s) => !s.futuro).length} stagion${stagioni.filter((s) => !s.futuro).length === 1 ? "e" : "i"} (solo vice)` : `${numStagioni} stagion${numStagioni === 1 ? "e" : "i"}`;
...
function buildRankingAccordion(stats) {
  const entries = Object.keys(stats).map((name) => {
    const stagioni = stats[name].stagioni;
    const numStagioni = stagioni.filter((s) => !s.isVice && !s.astaSolo && !s.futuro).length;
    const stagioniFull = stagioni.filter((s) => !s.isVice && !s.astaSolo && !s.futuro).sort((a, b) => b.annoInizio - a.annoInizio);
    return { name, numStagioni, stagioniFull, allStagioni: stagioni };
  });
  entries.sort((a, b) => b.numStagioni !== a.numStagioni ? b.numStagioni - a.numStagioni : a.name.localeCompare(b.name, "it"));

  const groups = {};
  entries.forEach((e) => { const k = e.numStagioni; if (!groups[k]) groups[k] = []; groups[k].push(e); });
    ...
    groups[num].forEach((e) => {
      ...
      const stagRows = e.stagioniFull.map((s) => `
          <div class="acc-season-row">
            ...
            <span class="acc-season-pos">${getPosLabel(s.pos)}</span>
          </div>`).join("");
      const viceRows = e.allStagioni.filter((s) => s.isVice || s.astaSolo).map((s) => {
        const badge = s.astaSolo ? '<span class="badge-asta">🎯 Asta</span>' : '<span class="badge-vice">VICE</span>';
        return `<div class="acc-season-row acc-vice-row">
            <span class="acc-season-year">${s.anno}</span>
            <img src="${s.logo}" class="acc-season-logo" onerror="this.src='images/default.png'">
            <span class="acc-season-team">${s.squadra}</span>
      ...
      html += `
        <div class="acc-item">
          <div class="acc-header" onclick="toggleAccordion('${uid}')">
            ...
            <div class="acc-name-area"><div class="acc-name">${e.name}</div><div class="acc-sub">${totalLabel}</div></div>
          ...
          <div class="acc-body" id="${uid}" style="display:none">
            ${stagRows}${viceRows}
            <div class="acc-scheda-btn" onclick="event.stopPropagation(); openMisterModal('${misterEncoded}')">👤 Vedi Scheda Completa</div>
          </div>
        </div>`;
    });
  });
...
function toggleFasce() {
  const hidden = document.getElementById("fasceSection").classList.contains("hidden");
  resetAllSections();
  if (hidden) { document.getElementById("fasceSection").classList.remove("hidden"); document.querySelector(".btn-fasce").innerText = "Torna alla Classifica"; renderFasce(); }
  else { document.getElementById("rankingSection").classList.remove("hidden"); }
}
...
function getFasciaLabel(fascia) {
  const map = { campione: "Campione 🏆", f1: "Fascia 1", f2: "Fascia 2", f3: "Fascia 3", ultimo: "Ultimo 🔴" };
  return map[fascia] || fascia;
}

function getFasciaPillClass(fascia) {
  const map = { campione: "pill-campione", f1: "pill-f1", f2: "pill-f2", f3: "pill-f3", ultimo: "pill-ultimo" };
  return map[fascia] || "pill-f3";
}

function getStripClass(fascia) {
  const map = { campione: "strip-campione", f1: "strip-f1", f2: "strip-f2", f3: "strip-f3", ultimo: "strip-ultimo" };
...
function buildObiettivoHtml(mister, fp, fd, inCorso, extra) {
  ...
  if (diff <= -2)
    return `<span class="obiettivo-superato"><span class="dot-obj dot-green"></span> Superato (+2 fasce)</span>`;
...
function renderFasce() {
  ...
  const stagioniFasce = fantaData.stagioni.filter((s) => s.anno === "2024-25" || s.anno === "2025-26");

  stagioniFasce.forEach((stagione) => {
    const extra = FASCE_EXTRA[stagione.anno] || {};
    const inCorso = extra.inCorso || false;
    const classifica = stagione.classifica.filter((t) => !t.isVice && !t.secondoAllenatore);
    ...
    classifica.forEach((team, i) => {
      ...
      let partenzaHtml = "", destinazioneHtml = "";

      if (extra.fasciaPartenza) {
        const fp = extra.fasciaPartenza[team.mister];
        const fd = extra.fasciaDestinazione[team.mister];
        const notaDest = extra.noteDestinazione ? extra.noteDestinazione[team.mister] || "" : "";
        const notaPart = extra.notePartenza ? extra.notePartenza[team.mister] || "" : "";
        ...
        if (fp && fd) {
          const ord = { campione: 0, f1: 1, f2: 2, f3: 3, ultimo: 4 };
          const diff = ord[fd] - ord[fp];
          if (diff < 0) moveHtml = `<span class="fascia-move move-up"><span class="dot-move dot-up"></span> Promozione</span>`;
          else if (diff > 0) moveHtml = `<span class="fascia-move move-down"><span class="dot-move dot-down"></span> Retrocessione</span>`;
          else moveHtml = `<span class="fascia-move move-same"><span class="dot-move dot-same"></span> Invariata</span>`;
        } else if (!fp && fd) {
      ...
      const obiettivoHtml = extra.fasciaPartenza
        ? buildObiettivoHtml(
            team.mister,
            extra.fasciaPartenza[team.mister],
            extra.fasciaDestinazione[team.mister],
            inCorso,
            extra
          )
        : `<span class="fascia-badge-pill ${getFasciaPillClass(fascia)}">${getFasciaLabel(fascia)}</span>`;

      const puntiDisplay = team.punti !== null && team.punti !== undefined ? team.punti : "null";
      html += `
        <tr style="animation-delay: ${i * 0.05}s">
...
const RISULTATI_2526 = [
  ...
  { g: 33, casa: "Lazio", gC: 1, gT: 4, tras: "Inter" },
  { g: 33, casa: "Milan", gC: 2, gT: 0, tras: "Napoli" },
  { g: 33, casa: "Juventus", gC: 2, gT: 1, tras: "Atalanta" },
  { g: 33, casa: "Bologna", gC: 0, gT: 1, tras: "Roma" },
  { g: 34, casa: "Juventus", gC: 1, gT: 1, tras: "Bologna" },
...
function buildStats2526() {
  ...
  [...source].sort((a, b) => a.g - b.g).forEach((r) => {
    ...
    else { stats[c].p++; stats[c].pC++; stats[c].ultimi.push("L"); }
    stats[t].pg++; stats[t].pgT++; stats[t].gf += gT; stats[t].gfT += gT; stats[t].gs += gC; stats[t].gsT += gC;
    if (gT > gC) { stats[t].v++; stats[t].vT++; stats[t].pt += 3; stats[t].ptT += 3; stats[t].ultimi.push("W"); }
