...
function renderSeason(anno) {
  ...
  classifica.forEach((team, i) => {
    ...
    const misterLabel = secondi.length > 0
...
function buildMisterStats() {
  ...
  if (!stats["Valentina Pozzi"].stagioni.find((x) => x.anno === "2025-26" && x.squadra === "Juventus")) {
    stats["Valentina Pozzi"].stagioni.push({
      ...
      squadra: "Juventus", logo: "images/juventus.png",
      pos: 5, parziale: false, isVice: true, secondoAllenatore: false,
      uscita: false, nota: "Viceallenatrice della Juventus"
    });
  }
  ...
  if (!stats["Giacomo Bot"].stagioni.find((x) => x.anno === "2023-24" && x.squadra === "Napoli")) {
    stats["Giacomo Bot"].stagioni.push({
      anno: "2023-24", annoInizio: 2023,
      squadra: "Napoli", logo: "images/napoli.png",
      pos: 8, parziale: false, isVice: true, secondoAllenatore: false,
      uscita: false, nota: "Viceallenatore di Mattia Minin"
    });
  }
  if (!stats["Giacomo Bot"].stagioni.find((x) => x.anno === "2024-25" && x.squadra === "Bologna")) {
    stats["Giacomo Bot"].stagioni.push({
      anno: "2024-25", annoInizio: 2024,
      squadra: "Bologna", logo: "images/bologna.png",
      pos: 4, parziale: false, isVice: true, secondoAllenatore: false,
      uscita: false, nota: "Viceallenatore di Federico Burello"
    });
  }
...
function openMisterModal(encodedName) {
  ...
  const torneiPartecipazioni = (fantaData.tornei || [])
    .map((t) => {
      ...
      if (futuro && !entry) {
        return {
          tipo: t.tipo, edizione: t.edizione,
          nazione: "TBD", logo: "images/default.png",
          fase: "In corso", punti: null, pos: null, futuro: true,
        };
      }
      return {
        tipo: t.tipo, edizione: t.edizione,
        nazione: entry.nazione, logo: entry.logo,
        fase: entry.fase, punti: entry.punti, pos: entry.pos,
      };
    })
    .filter(Boolean);
  ...
  document.getElementById("misterModalContent").innerHTML = `
    ...
    ${coppePartecipazioni.length > 0 ? `
      ...
      <div class="modal-trophies-list">${coppePartecipazioni.map((c) => {
        const faseColor = c.fase === "Campione" ? "fase-campione"
          : c.fase === "Finale" ? "fase-finale"
          : c.inCorso ? "fase-incorso" : "fase-eliminato";
        return `
    ...
    ${torneiPartecipazioni.length > 0 ? `
      ...
      <div class="modal-trophies-list">${torneiPartecipazioni.map((t) => {
        ...
        const tClass = (t.tipo === "FantaMundial" || t.tipo === "FantaMundiale")
          ? "torneo-entry mondiale-entry" : "torneo-entry europeo-entry";
        const faseColor = t.fase === "Campione" ? "fase-campione"
          : t.fase === "Finale" ? "fase-finale"
          : t.fase === "In corso" ? "fase-incorso" : "fase-eliminato";
...
function renderAttivita() {
  ...
  ordered.forEach((name) => {
    ...
    const cardClass = isAdmin ? " attivita-card-admin"
      : isOnlyVice ? " attivita-card-vice"
      : (isUscita && isActive) ? " attivita-card-uscita" : "";

    const avatarClass = isAdmin ? " attivita-avatar-admin"
      : isOnlyVice ? " attivita-avatar-vice"
      : (isUscita && isActive) ? " attivita-avatar-uscita" : "";

    const statusClassFinal = isActive
      ? isOnlyVice ? "attivita-status active-vice"
        : isUscita ? "attivita-status uscita"
        : "attivita-status active"
      : isOnlyVice ? "attivita-status retired-vice"
      : "attivita-status retired";

    const statusText = isActive
      ? isOnlyVice ? "🔁 Vice Attivo"
        : isUscita ? "🟡 Ultima Stagione"
        : "🟢 Attivo"
      : isOnlyVice ? "🔁 Vice Ritirato"
      : "🔴 Ritirato";
...
const FASCE_EXTRA = {
  ...
  "2025-26": {
    ...
    fasciaPartenza: {
      "Federico Burello": "f1", "Mattia Beltrame": "f1",
      "Kevin Di Bernardo": "f1", "Denis Mascherin": "f1",
      "Alex Beltrame": "f2", "Cristian Tartaro": "f2",
      "Lorenzo Moro": "f3", "Nicola Marano": "f3",
    },
    notePartenza: {
    ...
    fasciaDestinazione: {
      "Federico Burello": "f1", "Mattia Beltrame": "f1",
      "Kevin Di Bernardo": "f1", "Denis Mascherin": "f2",
      "Alex Beltrame": "f2", "Cristian Tartaro": "f3",
      "Lorenzo Moro": "f1", "Nicola Marano": "f3",
    },
    noteDestinazione: {
    ...
    classifica.forEach((team, i) => {
      ...
      if (extra.fasciaPartenza) {
        ...
        if (fp && fd) {
          const ord = { campione: 0, f1: 1, f2: 2, f3: 3, ultimo: 4 };
          const diff = ord[fd] - ord[fp];
          if (diff <= -2) obiettivoHtml = `<span class="obiettivo-superato"><span class="dot-obj dot-green"></span> Superato (+2 fasce)</span>`;
          else if (inCorso) obiettivoHtml = diff <= 0 ? `<span class="obiettivo-in-corso"><span class="dot-obj dot-gold"></span> In linea</span>` : `<span class="obiettivo-ko"><span class="dot-obj dot-red"></span> A rischio</span>`;
          else obiettivoHtml = diff <= 0 ? `<span class="obiettivo-ok"><span class="dot-obj dot-green"></span> Raggiunto</span>` : `<span class="obiettivo-ko"><span class="dot-obj dot-red"></span> Non raggiunto</span>`;
        } else { obiettivoHtml = "—"; }
      } else {
...
function buildStats2526() {
  ...
  const source = (fantaData.risultati2526 && fantaData.risultati2526.length > 0)
    ? fantaData.risultati2526 : RISULTATI_2526;
...
function renderStats2526(mode) {
  ...
  if (mode === 'forma') {
    ...
    teams.forEach((t, i) => {
      const pg=isC?t.pgC:isT?t.pgT:t.pg, v=isC?t.vC:isT?t.vT:t.v, n=isC?t.nC:isT?t.nT:t.n,
            p=isC?t.pC:isT?t.pT:t.p, pt=isC?t.ptC:isT?t.ptT:t.pt,
            gf=isC?t.gfC:isT?t.gfT:t.gf, gs=isC?t.gsC:isT?t.gsT:t.gs, dr=gf-gs;
